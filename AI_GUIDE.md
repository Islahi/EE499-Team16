...
6. Use `05_Meetings/Meeting_Minutes/EE499MeetingMinutes.docx` as the authoritative meeting-minutes template. Create a copy; never edit or overwrite the blank template.
7. Preserve the Word document's existing layout, tables, labels and signature fields. Fill the document using its exact section structure: **Attendees**, **Agenda**, **Discussion Points**, **Follow-up of the Last Meeting**, **Decisions Taken**, and **Actions to do before next meeting**. Do not replace this structure with a generic Markdown format.
8. Fill the header with the meeting date, Team 16 and the established project title. Mark actual attendance accurately in the existing attendee table; do not treat every listed person as present merely because the template contains their name or role. Leave signature cells untouched unless the user explicitly requests otherwise.
9. Save the result under `05_Meetings/Meeting_Minutes/` as `YYYY-MM-DD_EE499_Meeting_Minutes.docx`. Add a short distinguishing suffix when more than one meeting occurred on the same date. Create a Markdown companion only when the user explicitly asks for one.
10. Keep internal drafting labels, internal filenames and provenance notes out of generated documents. In particular, do not expose the terms “shadow design” or “shadow requirements”; express the underlying work accurately as the **implementation plan**, **design baseline**, or **project requirements**, as appropriate. This wording rule must not be used to claim that an internal plan was externally approved or independently validated.
11. For internal meetings where no advisor participated, omit the advisor from the attendee list and do not create advisor-specific sections, requests or follow-ups unless the user explicitly asks for them.
12. Treat every generated Word document as a draft requiring team verification. Record material uncertainties inside the most appropriate existing section without adding or redesigning template sections. Do not expose the raw source filename in the deliverable unless the user explicitly requests traceability. Render the completed DOCX to page images and visually verify every page before delivery.
13. After user/team confirmation, revise the document if requested. Record confirmed major technical decisions in `DECISIONS.md` and update `PROJECT_JOURNAL.md` only when the normal maintenance rules require it.
